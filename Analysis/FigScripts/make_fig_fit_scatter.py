...
# -*- coding: utf-8 -*-
"""
make_fig_fit_scatter_paper_dropin.py — drop-in “paper-ready” per pannelli fit scatter.
⟶ Mantiene *identica* data-area (W×H) con/ senza colorbar. Colorbar in gutter dedicato.
⟶ Nessun tight/ constrained. Nessuna modifica alla logica dati rispetto all'originale.
⟶ *Nuovo*: auto-espansione in larghezza per ospitare tick/label della colorbar, data-area invariata.
"""
...
CB_W_IN         = 0.10  # larghezza *barra* colorbar (slimmer)
CB_RIGHT_PAD_IN = 0.16  # spazio aggiuntivo *a destra* della colorbar per ticklabels e label

# === Formatting / export ===
CB_TICK_NBINS        = 4   # numero target di tick sulla colorbar
CB_TICK_DECIMALS = 2   # "al più" N decimali sui tick della colorbar
OUT_NAME_OVERRIDE    = None  # se non None, forza il nome base dei file (senza estensione)

# Micro-margini extra per far “respirare” le etichette senza toccare la data-area
LEFT_BUMP_IN   = 0.02  # applicato *in più* a LEFT_IN
BOTTOM_BUMP_IN = 0.02  # applicato *in più* a BOTTOM_IN
TOP_BUMP_IN    = 0.00  # opzionale
RIGHT_BUMP_IN  = 0.02  # applicato *solo* quando NON c'è la colorbar

# Stile/LaTeX
USE_TEX_MODE   = "latex"  # "latex", "latex_text", "pgf"
STYLE_BASE     = "paper_base.mplstyle"
STYLE_OVERLAY  = "overlay_latex.mplstyle"
# (Il percorso assoluto agli stili è derivato automaticamente da utils_style.)

# === Parametri script invariati (come nel file di partenza) ===
...
# === PLOTS (ripristinati come nella prima versione completa) ===
PLOTS: List[Dict[str, Any]] = [
    {"id":"raw_RRG_all_beta1", "model":"RRG", "subset_id":"b3e26b94c6ccd16e", "beta_kind":"raw", "kcol":"kFromChi_InBetween", "beta":1.,
      'out_name':"RRGFitVsN_rawbet1"},
     {"id":"raw_ER_Ngt30_beta1", "model":"ER","subset_id":"c2435bc67a3825b9","subset_label":"N>30", "beta_kind":"raw", "kcol":"kFromChi", "beta":1.,
      "include_unused": False,
      'out_name':"ERFitVsN_raw_bet1"},
     {"id":"raw_ER_Ngt30_beta1", "model":"ER","subset_id":"c2435bc67a3825b9","subset_label":"N>30", "beta_kind":"rescaled", "anchor":"M", "kcol":"kFromChi", "beta_rescaled_bin":1.,
      "include_unused": False, "colorbar": True,
      'out_name':"ERFitVsN_rescBetaM_bet1"},
...
def _fmt_cbar_tick(x: float, pos: int) -> str:
    # Rende al più CB_TICK_DECIMALS decimali, eliminando zeri finali
    try:
        s = f"{x:.{CB_TICK_DECIMALS}f}"
        s = s.rstrip('0').rstrip('.')
        return s
    except Exception:
        return str(x)
...
def _auto_expand_for_cbar(fig: plt.Figure, ax: plt.Axes, cbar: Colorbar,
    ...
    """Allarga la figura se i tick/label della colorbar escono dal canvas.
    Restituisce l'incremento in pollici (0 se non serve)."""
    fig.canvas.draw()
    r = fig.canvas.get_renderer()

    # Raccogli estensione massima in figure inches per gli elementi della cbar
    right_inches = 0.0
    # bounding box del cax
    bbox = cbar.ax.get_window_extent(renderer=r).transformed(fig.transFigure.inverted())
    right_inches = max(right_inches, bbox.x1 * fig.get_size_inches()[0])
    # ticklabels
    for tl in cbar.ax.get_yticklabels():
        bb = tl.get_window_extent(renderer=r).transformed(fig.transFigure.inverted())
        right_inches = max(right_inches, bb.x1 * fig.get_size_inches()[0])
    # label
    label = cbar.ax.yaxis.get_label()
    if label is not None:
        bb = label.get_window_extent(renderer=r).transformed(fig.transFigure.inverted())
        right_inches = max(right_inches, bb.x1 * fig.get_size_inches()[0])

    # Spazio desiderato = max(destra elementi, L+DATA_W+pad+Wcbar) + RIGHT FRAME
    desired_right_edge = right_inches + max(0.02, 0.00)  # piccolo extra
    current_w = fig.get_size_inches()[0]
    extra = max(0.0, desired_right_edge + R_frame - current_w)

    if extra > 1e-3:
        new_w = current_w + extra
        fig.set_size_inches(new_w, fig.get_size_inches()[1], forward=False)
        # riposiziona ax e cax per mantenere area-dati in pollici costante
        ax.set_position([L/new_w, B/fig_h, DW/new_w, DH/fig_h])
        # cax left/width non cambiano in pollici, ricalcolati in norm coords
        cax = cbar.ax
        cax.set_position([(L + DW + CB_PAD)/new_w, B/fig_h, CB_W/new_w, DH/fig_h])
        fig.canvas.draw()
        if VERBOSE:
...
def _plot_one(spec: Dict[str, Any]) -> Optional[Path]:
    ...
    with ustyle.auto_style(mode=USE_TEX_MODE,
        ...
        cb_requested = bool(spec.get("colorbar", False)) and (beta_kind == "rescaled")

        # Applica micro-bump ai margini L/B/T e R (solo senza colorbar)
        L0 = LEFT_IN   + LEFT_BUMP_IN
        B0 = BOTTOM_IN + BOTTOM_BUMP_IN
        T0 = TOP_IN    + TOP_BUMP_IN
        R0 = RIGHT_FRAME + (RIGHT_BUMP_IN if not cb_requested else 0.0)
        # Applica FIG_SCALE uniformemente a tutti i termini in pollici
        L = L0 * FIG_SCALE
        B = B0 * FIG_SCALE
        T = T0 * FIG_SCALE
        R_frame = R0 * FIG_SCALE
        DW = DATA_W_IN * FIG_SCALE
        DH = DATA_H_IN * FIG_SCALE
        CB_PAD = CB_PAD_IN * FIG_SCALE
        CB_W   = CB_W_IN * FIG_SCALE
        CB_RP  = CB_RIGHT_PAD_IN * FIG_SCALE

        # Dimensioni figura (base) — scalate
        fig_w = L + DW + (CB_PAD + CB_W + CB_RP if cb_requested else 0.0) + R_frame
        fig_h = B + DH + T

        # Crea figura+assi con area dati *esatta* tramite API del framework (valori *scalati*)
        fig, ax, _meta = uplot.figure_single_fixed(
            data_w_in=DW, data_h_in=DH,
            left_in=L, right_in=(R_frame + (CB_PAD + CB_W + CB_RP if cb_requested else 0.0)),
            bottom_in=B, top_in=T
        )
        fig.set_constrained_layout(False)  # vietato il constrained

        # Spessori/ ticks coerenti
        for sp in ax.spines.values():
            sp.set_linewidth(0.9)
        ax.tick_params(direction='out', length=3.2, width=0.8, pad=2.6)
        ax.xaxis.labelpad = 2.0  # leggero contenimento labelpad
        ...
        edge_palette = spec.get("edge_palette", {740:"red", 74:"orange", 73:"orange", 72:"purple", 71:"black", 70:"lightgreen", 0:"None", -2:"black"})
        ...
        # Prepara range per colorbar su valori *pre-binning*
        cb_enabled = False
        vmin = vmax = None
        if cb_requested:
            vals = np.concatenate([
        ...
        if not used_p.empty:
            ...
            ax.plot(x_line, y_line, linestyle="--", color="black", linewidth=1.6, zorder=0)
        if not fuu_p.empty:
        ...
        # ==================== COLORBAR (decoupled, solo cax) ===================
        if cb_enabled and (cax is not None):
            # Colorbar *decoupled* per evitare side-effects su ax principale
            mappable = plt.cm.ScalarMappable(norm=Normalize(vmin=vmin, vmax=vmax), cmap=cmap)
            mappable.set_array([])
            cbar = Colorbar(ax=cax, mappable=mappable)
            # label e ticks compatti + formatter con massimo N decimali
            ...
            cbar.ax.yaxis.set_major_formatter(FormatStrFormatter(f'% .{CB_TICK_DECIMALS}f'))
            cbar.ax.yaxis.labelpad = 1.6
            cbar.set_label(r"$\beta'$")

            # draw + *ripristino* posizione dell'axes principale (in unità normalizzate su fig_w corrente)
            fig.canvas.draw()
            ax.set_position([L/fig_w, B/fig_h, DW/fig_w, DH/fig_h])

            # auto-espansione: se tick/label escono, allarga figura e riposiziona
            expanded = _auto_expand_for_cbar(fig, ax, cbar, L, B, T, R_frame, fig_w, fig_h, DW, DH, CB_PAD, CB_W)
            if expanded > 0:
                fig_w += expanded
        # ======================================================================
        ...
        if _user_basename:
            out_base = out_dir / _sanitize(str(_user_basename))
        else:
        ...
        print(f"[FIG SIZE] fit_scatter: {W:.3f} × {H:.3f} in")
        bbox = ax.get_position()
        data_w = bbox.width * W; data_h = bbox.height * H
        right_total = (R_frame + (CB_PAD_IN + CB_W_IN + CB_RIGHT_PAD_IN if cb_requested else 0.0))
