...
if len(sys.argv) > 1:
    analysisVsSimTypesDict = { "ER":"ER", "RRG":"RRG"}
    ...
    for x in [[0.5,3],[0.5,4], [1,3], [1.0,4]]
    ]
    ...
    print(f"Analyzing all run groups of type {simType}. {len(selected_runGroups)} runs found.")

    i=0
    for runGroup in selected_runGroups:
        i+=1 
        analysis_path = os.path.join(parentFolder, runGroup[1])
        print(f"Analyzing run group #{i} out of {len(selected_runGroups)}\n")
