...
#ifdef STARTFIXEDANDHARDEND
#define startFixedAndHardEnd true
#else
#define startFixedAndHardEnd false
#endif
...
#define MCtimeMeasCadence 5 // mc-time to wait to do different mc-time dependent measurements

#define BICStories 100      // cadence of nStories whose m, qs, energy and for updated averages are printed
#define BICStoriesConf 1000 // cadence of nStories whose whole trajectory is printed

#include "../Generic/random.h"

#include "../MCtrajs/MCdyn_classi/interaction.h"
#include "../MCtrajs/MCdyn_classi/straj.h"
#include "../MCtrajs/MCdyn_classi/field.h"
...
int main(int argc, char **argv)
{
  ...
  int N = atoi(argv[2]);
  long int T = atol(argv[3]);
  int Np;
  if (T % MCtimeMeasCadence != 0)
  {
    cout << "Warning: T is not multiple of MCtimeMeasCadence. It will be rounded to the next multiple." << endl;
    T = (T / MCtimeMeasCadence + 1) * MCtimeMeasCadence;
  }
  int nStories = atoi(argv[4]);
  double beta = atof(argv[5]);
  double Hext = atof(argv[6]);
  double hin = atof(argv[7]);
  double hout = atof(argv[8]);
  if (startFixedAndHardEnd) // meaning we force initial conf in s_in and require q_out>=Q*
  {
    hin = 1.;
    hout = 1.;
  }
  ...
  double fracPosJ = atof(argv[12]);
  string graphID = argv[13];
  ...
  if (startFixedAndHardEnd)
    sprintf(buffer, "%li_%.4g_%.2g_inf_%i_inf", T, beta, Hext, Qstar);
  else
    sprintf(buffer, "%li_%.4g_%.2g_%.2g_%i_%.3g", T, beta, Hext, hin, Qstar, hout);
  ...
  if (argc == 16 || argc == 18)
  {
    double requiredBetaOfSExtraction = atof(argv[14]);
    int requiredQif = atoi(argv[15]);
    ...
    {
      // TOBEMODIFIED
      folder = makeFolderNameFromBuffer_ForCluster(folder + "Prova/PT_" + to_string(result.second.first).substr(0, 4) + "_" + to_string(result.second.second.first).substr(0, 4) + "__" + to_string(result.second.second.second.first) + "_" + to_string(result.second.second.second.second) + "/PathsMCs/", string(buffer) + "_sigma" + to_string(sigma), sstart); // For Cluster
    }
    else
    {
      folder = makeFolderNameFromBuffer_ForCluster(folder + "Prova/PT_" + to_string(result.second.first).substr(0, 4) + "_" + to_string(result.second.second.first).substr(0, 4) + "__" + to_string(result.second.second.second.first) + "_" + to_string(result.second.second.second.second) + "/PathsMCs/", string(buffer), sstart); // For Cluster
    }
  }
  else
  {
    ...
    if (argc == 17 || argc == 19)
    {
      folder = makeFolderNameFromBuffer_ForCluster(folder + "Prova/PathsMCs/", string(buffer) + "_sigma" + to_string(sigma), sstart); // For Cluster
    }
    else
    {
      folder = makeFolderNameFromBuffer_ForCluster(folder + "Prova/PathsMCs/", string(buffer), sstart); // For Cluster
    }
  }
  ...
  */
  ...
  // END of initialization of graph, reference configurations

  // START of the proper MC simulation

  Np = int(T / MCtimeMeasCadence) + 1;
  /*
  info.first += "mcTotal=" + to_string(MC) + " MCmeas=" + to_string(MCmeas) + " MCprint=" + to_string(MCprint) + " Np=" + to_string(Np) + "\n\n";
  info.second += "30 " + to_string(MC) + " " + to_string(MCmeas) + " " + to_string(MCprint) + " " + to_string(Np) + "\n";
  ...
  */
  ...
  nomefile = folder + "/storiesStats.dat";
  ...
  */
  ...
  for (long nStory = 1; nStory <= nStories; nStory++)
  {
    s = s_in;
    for (int i = 0; i < Qs.size(); i++)
      Qs[i] = QsStart[i];

    for (long mc = 0; mc <= T; mc++)
    {
      // cout << "Qout vale" << Qs[1] << " Qstar vale " << Qstar << endl;
      //  MC move
      for (int i = 0; i < N; i++)
      {
        int I = (int)(N * Xrandom());
        ...
        Qs[1] -= s[I] * s_out[I];
        Qs[2] -= s[I];
        MCStep_withGraph(s,  I,  N,  Graph,  beta,  Hext, randomField);
        Qs[0] += s[I] * s_in[I];
        Qs[1] += s[I] * s_out[I];
        Qs[2] += s[I];
      }

      if (mc % MCtimeMeasCadence == 0)
      {

        for (int j = 0; j < mileStones.size(); j++)
        {
          fracOfTrajsInCone.first[j][mc / MCtimeMeasCadence] += (Qs[0] >= mileStones[j] ? 1 : 0);
          fracOfTrajsInCone.second[j][mc / MCtimeMeasCadence] += (Qs[1] >= mileStones[j] ? 1 : 0);
        }
      }
    }
    /*
    if (nStory % mcForIntegratedMeasuring < averagingMCForIntegratedMeasuring)
    {
  ...
  for (int i = 0; i < 3; i++)
    qfin_av[i] += ((double)Qs[i] / (double)N);
  chifin_av += (Qs[1] > Qstar ? Qs[1] - Qstar : 0) / ((double)N);

  if (!startFixedAndHardEnd)
    probafin_av += (Qs[1] >= Qstar ? exp(2 * hout) : 1);
  else
    probafin_av += (Qs[1] >= Qstar ? 1 : 0);

  counter++;

  // print on file
  if ((mc % MCprint == 0) && ((mc == 0)))
  {
    ...
    fileM << hout << " " << chifin_av / counter << " " << Qstar << " " << probafin_av / counter << " " << beta << endl;
    fileM.close();
   }
 }
 ...
 */
    ...
    if (nStory % 20 == 0)
    {
      ...
      for (int i = 0; i < Np; i++)
      {
        ...
        */
        for (int j = 0; j < mileStones.size(); j++)
          fileM << fracOfTrajsInCone.first[j][i] / (double)nStory << " ";

        for (int j = 0; j < mileStones.size(); j++)
          fileM << fracOfTrajsInCone.second[j][i] / (double)nStory << " ";
        fileM << endl;
      }
    }
  }
