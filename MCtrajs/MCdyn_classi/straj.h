...
public:
  ...
  void set_s0(int _s0) { s0 = _s0; }
  void resize(int n) { jumps.resize(n); }
  void insert_front(int _tj) { jumps.insert(jumps.begin(), _tj); }
  void push_back(int _tj) { jumps.push_back(_tj); }
  void push_back(disc_straj t)
  {
    jumps.insert(jumps.end(), t.begin(), t.end());
  }
  void clear() { jumps.clear(); }
  ...
  // output
  int size() { return jumps.size(); }
  int operator[](int i) { return jumps[i]; }
  int back() { return jumps.back(); }
  vector<int>::iterator begin() { return jumps.begin(); }
  vector<int>::iterator end() { return jumps.end(); }
  double s_av() // returns the spin average value over the trajectory (RC)
  {
    if (jumps.empty())
      return s0;
  ...
  friend ostream &operator<<(ostream &o, const disc_straj &a)
  {
    int currentn = a.s0;
    o << "0 " << currentn << "\n";
    for (int i = 0; i < a.jumps.size(); i++)
